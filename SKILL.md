...
- **Two things are spent, metered differently.** The *window* (context) is per session and is what §1 is about. The *subscription* is per billing pool, and a subagent on the same provider account draws on the same pool as the session that dispatched it. A same-provider dispatch relieves the window and the attention, never the account's meter; only a workhorse on a different pool (another provider, another account, a separately capped API key) is disjoint. Reason "delegate to keep this window lean," and route to a different pool when the meter itself is the constraint. Two sessions on one account read one balance; summing per session invents capacity.
- **The meter moves routing and timing, never the bar.** A low balance is a reason to transfer a unit or wait for a reset; it is never a reason to trim a gate, a read, or a review. A necessary burn (a full test run, an adversarial read, a long comprehension pass) is named and spent. Trimming quality to save tokens is this contract's vice, and the non-hampering clause above is the rule, not a preference.
- **Peers are not subagents.** Another agent sharing a room and a working copy with you (a different model, harness, or machine) is addressed, claimed to, and verified — never dispatched, and its cost is its own pool's. This contract's verdicts apply inside one seat, to that seat's subagents and workhorses; the question of whether a unit goes to a peer is one of ownership and independent verification, never token arithmetic.
...
- **A "fork"-style dispatch that inherits the whole conversation re-bills the entire accumulated window into the child** — the exact cost this contract exists to avoid. Fork only when the child genuinely needs the conversation (a continuation of your own reasoning), never as a delegation shape. Where the harness offers per-agent filesystem isolation (a separate worktree per writer), give it to every concurrent writer; the file-ownership partition below still governs what each may touch, and read-only agents stay on the shared tree.
- **A shared, injected report path is a clobber surface.** If your dispatch tooling routes every agent's report to one conventional path under the working directory, parallel units into one directory overwrite each other (last finisher wins), and the injected convention tends to win over a report path stated in the brief. Give every dispatch its own explicit report path through the tool's own flag, and make the brief name the same path — when the flag and the brief agree there is nothing to conflict; when they disagree an obedient agent follows one and the wrapper reports a false failure on a unit whose work was complete (both bitten in one long-running environment; zero clobbers after the fix across two multi-unit campaigns).
- **Route by capability fit, and say so in the brief.** When a unit's surface is one a given model is known to refuse or misclassify on legitimate work, send it to a model that does it without incident, by explicit `model:`, the way you would choose a numerics library for a numerics job. That is model selection. What it is not: a digest kept to stay below a threshold, vocabulary withheld from a window, or any rephrasing meant to get past a safeguard; a method that manages a control's view of the work is not retained, whatever the footing.
- **Classify a fault before answering it.** Transient API, auth, rate-limit, context and tool failures get a bounded resume from the checkpoint (try resume before re-dispatch — recovery is one message where a fresh dispatch is a whole brief). An explicit policy refusal on the material is a health event, not a retry: record it with the checkpoint, reassign the unit to another eligible model, and reassess scope and authorization — never re-dispatch, rephrase or re-route with the purpose of getting past a safeguard. A dead agent (process gone) and a stalled one (alive, no progress past its bound) are different states; process liveness is not health.
...
- **Number-crunching units carry three mandatory brief riders.** (1) *Full local parallelism*: the brief requires the implementation to size itself to the hardware — a worker pool spanning the cores over the embarrassingly-parallel axis, GPU compute where the workload maps. A single-threaded hour-scale crunch is a brief-authoring failure, not a builder choice (field-exhibited: a matrix crunch pegging one core of a many-core workstation while the work queue waited). (2) *A milestone progress file*: one appended line per completed stage beside the results, plus partial results where the format allows — on thin-telemetry harnesses the progress file is the orchestrator's only liveness channel, and the non-destructive hang check is its tail plus a two-sample CPU-delta on the compute process (~8s apart; flat = hung, climbing = crunching). Probe before ever considering a kill: the check costs seconds, a re-run costs the whole crunch. (3) *Language by workload*: the environment's house scripting language is glue, not a numerics default — matrix/eigen/sieve-heavy units belong in BLAS/LAPACK-backed stacks (NumPy/SciPy), native code (Rust/C with SIMD), Julia, or GPU kernels per fit. The gate contract is unchanged; only the engine under it changes.
...
### 7a. Estimates are not meters

Anything computed from local transcripts — per-model token totals, list-price costs, a rolling-window throttle that proxies a cap from the largest block ever completed — is an estimate. It is useful for seeing delegation compound (the orchestrator's cache-read spend bending toward cheap workhorse tokens) and it is never a reading of remaining allowance. A reading is keyed to a billing pool and carries its source and its age; a clock reaching a reset time is not a reading either — the balance is "reset due" until a fresh one confirms it. Route on readings; watch estimates.
